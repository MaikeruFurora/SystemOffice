...
var ERROR_LOGIN_ADMIN_USER_REPEAT = 108009;
        ...
        $.ajax({
            ...
            error: function (XMLHttpRequest, textStatus, errorThrown) {
                try {
                    ...
                    log.error(
                        "MAIN : XMLHttpRequest.readyState = " +
                            XMLHttpRequest.readyState
                    );
                    log.error(
                        "MAIN : XMLHttpRequest.status = " +
...
function checkGotoHome(enable) {
    if (!enable) {
        gotoPageWithoutHistory(g_publicpage + ".html");
    }
}
function checkModuleEnable(current_page) {
    switch (current_page) {
        ...
        case "sntp":
            checkGotoHome(
                checkObjectExist(g_PageUrlTree, "settings", "system", "sntp")
            );
            break;
        ...
        case "dhcp":
            checkGotoHome(
                checkObjectExist(g_PageUrlTree, "settings", "wlan", "dhcp")
            );
            break;
        case "serverbasic":
            checkGotoHome(
...
function setMainMenu() {
    ...
    if (
        ...
        if (MenuNumber < g_MainMenuNumber) {
            $("#wifisettings").children().text(wan_wifisettings_title);
            MenuNumber++;
        } else if (
            g_module.statistic_enabled &&
            MenuNumber >= g_MainMenuNumber
        ) {
            $("#menu_wifisettings").remove();
            MenuNumber++;
            Add_more_menu(
                "wifisettings",
                "wifisettings.html",
                wan_wifisettings_title
            );
        } else {
            $("#menu_wifisettings").remove();
        }
...
function isneedsystemsettings() {
    if (
        g_module.powersave_enabled ||
        g_module.fastboot_enabled ||
        g_USBtetheringSwitch == USBTETHERING_ON
    ) {
        return true;
    }
    return false;
        ...
        $(".main_left").load("leftmenu.html #settings_menu", function () {
            ...
            if (g_module.vpn_enabled) {
                if (checkLeftMenu(g_PageUrlTree.settings.vpn)) {
                    ...
                    if (checkLeftMenu(g_PageUrlTree.settings.vpn.vpnstatus)) {
                        $("#label_vpn_status").attr(
                            "title",
                            IDS_vpn_label_vpn_status
                        );
                    ...
                    if (checkLeftMenu(g_PageUrlTree.settings.vpn.vpnsettings)) {
                        $("#label_vpn_setting").attr(
                            "title",
                            IDS_vpn_label_setting
                        );
            ...
            if (
                ...
                if (checkLeftMenu(g_PageUrlTree.settings.voip.sipadvance)) {
                    $("#label_sip_advanced").attr(
                        "title",
                        IDS_VOIP_menu_label_sipadvance
                    );
                    $("#label_sip_advanced").text(
                        IDS_VOIP_menu_label_sipadvance
                    );
                } else {
                    $("#sipadvance").remove();
                ...
                if (
                    checkLeftMenu(g_PageUrlTree.settings.voip.voiceadvanced) &&
                    g_module.voip_adcance_enable
                ) {
            ...
            if (checkLeftMenu(g_PageUrlTree.settings.security)) {
                ...
                if (
                    g_module.aclui_enabled &&
                    checkLeftMenu(g_PageUrlTree.settings.security.acl)
                ) {
                    $("#label_acl_settings").attr(
                        "title",
                        firewall_label_acl_settings
                    );
            ...
            if (checkLeftMenu(g_PageUrlTree.settings.system)) {
                ...
                if (
                    g_module.sntp_enabled &&
                    checkLeftMenu(g_PageUrlTree.settings.system.sntp)
                ) {
                ...
                if (
                    checkLeftMenu(g_PageUrlTree.settings.system.modifypassword)
                ) {
                ...
                if (
                    g_module.tr069_enabled &&
                    checkLeftMenu(g_PageUrlTree.settings.system.tr069)
                ) {
                ...
                if (
                    g_module.logenabled &&
                    checkLeftMenu(g_PageUrlTree.settings.system.logview)
                ) {
                ...
                if (
                    (g_module.static_route_enabled ||
                        g_module.static_route6_enabled) &&
                    checkLeftMenu(g_PageUrlTree.settings.system.staticroute)
                ) {
    ...
    if ($(".main_left").children().is("#update_menu")) {
        if (
            ...
            (checkLeftMenu(g_PageUrlTree.update.update_local) ||
                checkLeftMenu(g_PageUrlTree.update.update_online))
        ) {
            if ($(".main_left").children().is("#update_menu")) {
...
function showFooter() {
    if (g_needFooter) {
        $("#footer").load("footer.html img, span, rightcorner", function () {
            ...
            $("#footer span").html(
                "&nbsp;" + temp + "<div class='rightcorner'></div>"
            );
            ...
            $("#footer").prepend(
                "<a id='pravacy_policy' class='clr_gray' target='_blank'>" +
            ...
            getAjaxData("api/device/device-feature-switch", function ($xml) {
                ...
                if (ret.type == "response") {
                    ...
                    if (g_copyright_enabled == "1") {
                        $("#footer span").after(
                            "<span id='copyright_notice'><a href='/html/OPEN_SOURCE_SOFTWARE_NOTICE.pdf' class='clr_gray'>" +
...
$(document).ready(function () {
    ...
    showFooter();
    $(
        ".button_wrapper:not(#login_btn,#connect_btn,#disconnect_btn,#cancel_btn)"
    ...
    if (
        ...
        getAjaxData(
            ...
            function ($xml) {
                var ret = xml2object($xml);
                if ("response" == ret.type) {
                    var BridgeModeStatus = parseInt(
                        ret.response.bridgemode,
                        10
                    );
...
function setSettingMenu(obj) {
    $.each(obj, function (i, n) {
        if (typeof n == "object" && !g_isReady) {
            ...
            $("#settings").attr("href", "deviceinformation.html");
            g_isReady = true;
    ...
    if ($.trim($("#header").html()) == "" || $("#header").html() == null) {
        $("#header").load("header.html", function () {
            ...
            if (
                ...
                $(".logo").after(
                    "<div class='help'><span><select id='lang' style='display:none'></select></span><span class = 'clr_bold_a' style='display:none'><a id=help_url href='#' target='_blank'><span id='help_span'></span></a></span><span><span class='username'  id='username_span' style='display:none'></span></span><span>&nbsp;</span><span><span class='logout'><span id='logout_span'></span></span></span></div>"
                );
            } else {
                $(".logo").after(
                    "<div class='language'><span><select id='lang' style='display:none'></select></span></div>"
                );
            }
            setLangList();
            showCurrentLanguage();
                ...
                getAjaxData(
                    ...
                    function ($xml) {
                        ...
                        if (ret.type == "response") {
                            ...
                            if ("admin" != g_loginStatus.Username) {
                                $("#menu_sms").show();
                                $("#menu_update").remove();
                                $("#menu_wifisettings").show();
                                $(".nav01").remove();
                            } else {
                                $("#menu_update").show();
                                $(".nav02").remove();
    ...
    if ($.trim($("#header").html()) == "" || $("#header").html() == null) {
        ...
        $("#header").load("header.html", function () {
            if ($.browser.msie) {
                $(".help div").remove();
                $(".help").append(
                    " <span><select id='lang' style='display:none'></select></span><span class = 'clr_bold_a'><a id=help_url href='#' target='_blank'><span id='help_span'></span></a></span>" +
            ...
            var menu_id =
                "#menu_statistic,#menu_wifisettings,#menu_sms,#menu_connection_settings a,#menu_ussd,#menu_update,#menu_settings,.nav01,.nav02,#profile_settings a,#menu_pb,#menu_stk, #wifi_turnOff_button,#sambasettings,#sambasharinguser,#dlna,#menu_appmanagement";
...
function getLoginStatus_Status(callback, redirectDes) {
    var flag = true;
    if (g_needToLogin) {
        getAjaxData(
            "api/user/state-login",
...
function showloginDialog() {
    ...
    dialogHtml +=
        "                   <span><input  type='password'  class='input' id='password' maxlength='15'/></span>";
    dialogHtml += "               </div>";
    dialogHtml += "               </div>";
    dialogHtml += "        </div>";
    ...
    $.ajax({
        ...
        error: function (XMLHttpRequest, textStatus) {
            try {
                ...
                log.error(
                    "MAIN : XMLHttpRequest.status = " + XMLHttpRequest.status
                );
                log.error("MAIN : textStatus " + textStatus);
            } catch (exception) {
                log.error(exception);
        ...
        success: function (data) {
            ...
            if ("error" == ret.type) {
                if (ERROR_WRONG_SESSION_TOKEN == ret.error.code) {
                    log.error(
                        "Main: getajax" + this.url + "session token error"
                    );
                    gotoPageWithoutHistory(HOME_PAGE_URL);
                    return;
                }
...
function xssVerifyProtocal(url) {
    if (!url) {
        return "";
    }
    if (url.toUpperCase().substring(0, 4) !== "HTTP") {
        url = "http://" + url;
    }
    return url;
}
        ...
        success: function (data) {
            ...
            if (
                ...
                if (ERROR_WRONG_SESSION_TOKEN == xml_ret.error.code) {
                    log.error(
                        "Main: saveAjaxDate" + this.url + "session token error"
                    );
                    gotoPageWithoutHistory(HOME_PAGE_URL);
                    return;
                }
                if (ERROR_WRONG_SESSION == xml_ret.error.code) {
                    log.error(
                        "Main: saveAjaxDate" + this.url + "session  error"
                    );
                    gotoPageWithoutHistory(HOME_PAGE_URL);
                    return;
                }
...
function getGlobalFeatures() {
    ...
    var current_href_main = window.location.href;
    if (current_href_main.indexOf("?url") > 0) {
        current_href_main = current_href_main.substring(
            0,
            current_href_main.indexOf("?url")
        );
        current_href_main = current_href_main.substring(
            current_href_main.lastIndexOf("/") + 1
        );
        current_href_main = current_href_main.substring(
            0,
            current_href_main.indexOf(".")
        );
    } else {
        current_href_main = current_href_main.substring(
            current_href_main.lastIndexOf("/") + 1
        );
        if (current_href_main.lastIndexOf("?") > 0) {
            current_href_main = current_href_main.substring(
                0,
                current_href_main.lastIndexOf("?")
            );
            current_href_main = current_href_main.substring(
                0,
                current_href_main.indexOf(".")
            ...
            if (
                current_href_main == "smsinbox" ||
                current_href_main == "wifisettings"
            ) {
                current_href_main = location.search.substring(1);
            }
        } else {
            current_href_main = current_href_main.substring(
                0,
    ...
    getAjaxData(
        ...
        function ($xml) {
            var ret = xml2object($xml);
            if ("response" == ret.type) {
                if ("user" === ret.response.Username) {
                    if (
                        current_href_main == "update" ||
                        current_href_main == "update_local"
                    ) {
                    ...
                    if (
                        current_href_main == "smsinbox" ||
                        current_href_main == "wifisettings" ||
                        current_href_main == "messagesettings" ||
                        current_href_main == "smssent" ||
                        current_href_main == "smsdrafts"
                    ) {
                        gotoPageWithoutHistory("home.html");
                    }
                    $(".nav02").remove();
        ...
        function ($xml) {
            ...
            if (ret.type == "response") {
                ...
                g_module = {
                    ...
                    ota_enabled:
                        g_moduleswitch.ota_enabled == "1" ? true : false,
                    wifioffload_enable:
                    ...
                    tr069_enabled:
                        g_moduleswitch.tr069_enabled == "1" ? true : false,
                    antenna_enabled:
                        g_moduleswitch.antenna_enabled == "1" ? true : false,
                    voip_adcance_enable:
                        voiceadvancesetting == "1" ? true : false,
                    aclui_enabled:
                        g_moduleswitch.aclui_enabled == "1" ? true : false,
    ...
    getAjaxData(
        ...
        function ($xml) {
            ...
            if (ret.type == "response") {
                ...
                g_wifi = {
                    stawpsenabled:
                        g_wifiFeatureSwitch.stawpsenabled == "1" ? true : false,
                };
            }
        },
        {
...
function showPasswordStrength(idOfTextbox) {
    var strengthBar =
        '<div class="pwd_strength"><table><tr><td style="color:red;">' +
        IDS_psw_strength_label +
        common_colon +
        "</td>";
    strengthBar +=
        '<td><table class="pwd_strength_tb" border="0" cellpadding="0" cellspacing="0"><tr><td id="pwd_strength_low" width="33.3%" align="center">' +
        IDS_psw_strength_low +
        "</td>";
    strengthBar +=
        '<td id="pwd_strength_mid" width="33.3%" align="center">' +
        IDS_psw_strength_mid +
        "</td>";
    strengthBar +=
        '<td id="pwd_strength_hig" width="33.3%" align="center">' +
    ...
    if (firstTimeMontitoringStatusQuery == "0") {
        ...
        if ($.browser.opera && $.browser.version == "11.00") {
            ...
            getAjaxData(
                ...
                function ($xml) {
                    log.debug(
                        "MAIN : getDeviceStatus() get monitoring success"
                    );
                    var ret = xml2object($xml);
                    if (ret.type == "response") {
                        G_MonitoringStatus = ret;
...
function initialStatusIcon() {
    ...
    $("#sim_signal_gif").qtip({
        content:
            "<b>" + STATUS_BAR_ICON_STATUS.sim_signal_tooltip_state + "</b>",
        position: {
            corner: corner,
        },
        style: {
    ...
    if (header_ret != null && header_ret.type == "response") {
        ...
        if (
            ...
            if (MACRO_PIN_REQUIRED == g_main_convergedStatus.SimState) {
                $("#sim_signal_gif").html(
                    "<img onload = 'fixPNG(this)' src = '../res/sim_disable.png ' 0 0 no-repeat>"
                );
            ...
            if (!sign_enable) {
                ...
                $("#SIMStatus").text(sim_status_normal);
            }
        }
        if (sign_enable) {
            ...
            if (
                ...
                switch (header_icon_status.SignalStrength) {
                    ...
                    case MACRO_EVDO_LEVEL_THREE:
                        $("#sim_signal_gif").html(
                            "<img onload = 'fixPNG(this)' src = '../res/signal_" +
    ...
    function ap_station_disabled() {
        switch (header_icon_status.ConnectionStatus) {
            case MACRO_CONNECTION_CONNECTED:
                ...
                if (
                    ($.browser.msie && $.browser.version == "6.0") ||
                    $("#wan_gif").html().indexOf("wan_disable.png") < 0
                ) {
                            ...
                            if (
                                ...
                                if (
                                    ...
                                    if (g_up_connection_status == "0") {
                                        $("#wan_gif").html(
                                            "<img onload = 'fixPNG(this)' src = '../res/wan_up.png' 0 0 no-repeat>"
                                        );
                                    ...
                                    if (g_down_connection_status == "0") {
                                        $("#wan_gif").html(
                                            "<img onload = 'fixPNG(this)' src = '../res/wan_down.png' 0 0 no-repeat>"
                                        );
                                        g_disable_connection_status = "0";
                                        g_up_down_connection_status = "0";
                                    ...
                                    if (g_up_down_connection_status == "0") {
                                        $("#wan_gif").html(
                                            "<img onload = 'fixPNG(this)' src = '../res/wan_up_down.png' 0 0 no-repeat>"
                                        );
                                        g_disable_connection_status = "0";
                                    ...
                                    if (g_disable_connection_status == "0") {
                                        $("#wan_gif").html(
                                            "<img onload = 'fixPNG(this)' src = '../res/wan_disable.png' 0 0 no-repeat>"
                                        );
    ...
    if (
        ...
        if (station_ret != null && station_ret.type == "response") {
            ...
            if (
                ...
                $("#station_gif")
                    .html()
                    .indexOf("station_" + apSignal + ".png") < 0
            ) {
                $("#station_gif").html(
...
function getBatteryLevel(str) {
    if (g_coulometer_status == "1") {
        ...
        if (0 < strTemp && strTemp <= 10) {
            $("#battery_gif").html(
                "<img onload = 'fixPNG(this)' src = '../res/battery_level_1.png' 0 0 no-repeat>"
            );
        } else if (10 < strTemp && strTemp <= 23) {
            $("#battery_gif").html(
                "<img onload = 'fixPNG(this)' src = '../res/battery_level_2.png' 0 0 no-repeat>"
            );
        } else if (23 < strTemp && strTemp <= 34) {
            $("#battery_gif").html(
                "<img onload = 'fixPNG(this)' src = '../res/battery_level_3.png' 0 0 no-repeat>"
            );
        } else if (34 < strTemp && strTemp <= 45) {
            $("#battery_gif").html(
                "<img onload = 'fixPNG(this)' src = '../res/battery_level_4.png' 0 0 no-repeat>"
            );
        } else if (45 < strTemp && strTemp <= 56) {
            $("#battery_gif").html(
                "<img onload = 'fixPNG(this)' src = '../res/battery_level_5.png' 0 0 no-repeat>"
            );
        } else if (56 < strTemp && strTemp <= 67) {
            $("#battery_gif").html(
                "<img onload = 'fixPNG(this)' src = '../res/battery_level_6.png' 0 0 no-repeat>"
            );
        } else if (67 < strTemp && strTemp <= 78) {
            $("#battery_gif").html(
                "<img onload = 'fixPNG(this)' src = '../res/battery_level_7.png' 0 0 no-repeat>"
            );
        } else if (78 < strTemp && strTemp <= 89) {
            $("#battery_gif").html(
                "<img onload = 'fixPNG(this)' src = '../res/battery_level_8.png' 0 0 no-repeat>"
            );
        } else if (89 < strTemp && strTemp <= 100) {
            $("#battery_gif").html(
                "<img onload = 'fixPNG(this)' src = '../res/battery_level_9.png' 0 0 no-repeat>"
            );
        } else {
            $("#battery_gif").html(
                "<img onload = 'fixPNG(this)' src = '../res/battery_level_0.png' 0 0 no-repeat>"
            );
        }
    } else {
        switch (str) {
            ...
            case MACRO_BATTERY_LEVEL_THREE:
                ...
                $("#battery_gif").html(
                    "<img onload = 'fixPNG(this)' src = '../res/battery_level_" +
                        temp +
                        ".png' 0 0 no-repeat>"
                );
                break;
...
function addStatusListener(funName) {
    ...
    log.debug(
        "MAIN : addStatusListener(" +
            funName +
            "), currently " +
            STATUS_LISTENER_FUN_LIST.length +
            " listener."
    );
        ...
        saveAjaxData(
            ...
            function ($xml) {
                ...
                if (isAjaxReturnOK(ret)) {
                    ...
                    $("#logout_span").text(common_logout);
                    if (
                        "pincoderequired" == current_href ||
                        "pukrequired" == current_href
                    ) {
                        $("#link_login_home").show();
                        $("#link_login_publicpage").hide();
                    }
                    ...
                    if (ret.type == "error") {
                        ...
                        if (ret.error.code == ERROR_LOGIN_PASSWORD_WRONG) {
                            ...
                            ret.error.code == ERROR_LOGIN_ALREADY_LOGIN
                        ) {
                            showErrorUnderTextbox(
                                "password",
                                common_user_login_repeat
                            );
                            $("#username").focus();
                            $("#username").val("");
                            $("#password").val("");
                        } else if (
                            ret.error.code == ERROR_LOGIN_USERNAME_WRONG
                        ) {
                            showErrorUnderTextbox(
                                "username",
                            ...
                            ret.error.code == ERROR_LOGIN_ADMIN_USER_REPEAT
                        ) {
                            showErrorUnderTextbox("password", IDS_login_repeat);
                            $("#username").focus();
                            $("#username").val("");
                            $("#password").val("");
                        }
...
function checkPWStrength(passValue) {
    ...
    if (sPWLength < 6 || (sPWModes == 1 && sPWLength < 10)) {
        ...
        (sPWModes == 2 && sPWLength >= 6) ||
        (sPWModes == 1 && sPWLength >= 10)
    ) {
    ...
    if (2 == arguments.length) {
        if (
            !String(passValue).localeCompare(arguments[1]) ||
            !String(passValue).localeCompare(
                arguments[1].split("").reverse().join("")
            )
        ) {
            ret = MACRO_PASSWORD_LOW;
        }
...
function getUserManualUrl() {
    if (g_needHelp) {
        ...
        if (supportFlag == false) {
            current_language = usermanual_language_list.default_language.replace(
                /-/,
                "_"
            );
        }
...
function regURL(str) {
    ...
    return str.replace(matchURL, function ($1) {
        ...
        return (
            "<a href='" +
            $1_href +
            "' style='text-decoration:underline;' target='_blank' onclick='javascript:stopBubble(event)'>" +
            $1 +
            "</a>"
        );
    });
